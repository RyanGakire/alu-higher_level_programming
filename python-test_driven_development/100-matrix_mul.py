...
This module holds a single function, ``matrix_mul``, that multiplies
two matrices together after validating both of them, and returns the
resulting matrix as a new list of lists.
"""


def matrix_mul(m_a, m_b):
    """Return the matrix product of ``m_a`` by ``m_b``.
    ...
    Returns:
        A new matrix holding the product of ``m_a`` by ``m_b``.
    ...
    for row in m_a:
        ...
        for column in range(len(m_b[0])):
            ...
            for index in range(len(m_b)):
                total += row[index] * m_b[index][column]
